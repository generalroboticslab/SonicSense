...
class get_overall_accuracy(object):
    def __init__(self,params,config_path):
    ...
    def calculate_accuracy(self,split,k_neighbor,num_loop,mim_occurence):
        ...
        #get f1 score
        average_f1_score = self.get_average_f1_score(torch.from_numpy(np.array(y_hat)),torch.from_numpy(np.array(y)))
        acc=[]
    ...
    def get_average_f1_score(self,y_hat,y):
        metric = MulticlassConfusionMatrix(num_classes=self.num_class)
        cm=metric(y_hat,y)
        confusion_matrix_computed = cm.detach().cpu().numpy().astype(int)
        precision_list = []
    ...
    def get_best_params_from_val(self):
        ...
        for k in range(1,9):
            ...
            print('current bast valid average f1 score:', max(acc))
        print('final best valid average f1 score:', max(acc))
    ...
    def get_best_test_acc(self):
        ...
        print('best test average f1 score:', acc_balanced)
        return acc_balanced